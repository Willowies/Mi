			...
			<div class="middle-content">
				...
				<div class="right-content">
					<div class="content-head">
						...
						<hr style="width: 600px; margin: 0; text-align: left; border-color: #ddd;" />
						<div class="head-details">
							<ul class="head-list">
								<li <c:if test="${type == 1}">class="first"</c:if>>
									<a href="getAllGroupOrder.action">全部有效订单</a>
								</li>
								<li <c:if test="${type == 2}">class="first"</c:if>>
									<a id="J_unpaidTab" href="getGroupOrderWaitPaid.action">待支付</a>
								</li>
								<li <c:if test="${type == 5}">class="first"</c:if>>
									<a href="getGroupOrderWaitBuilt.action">待成团</a>
								</li>
								<li <c:if test="${type == 3}">class="first"</c:if>>
									<a id="J_sendTab" href="getGroupOrderWaitTaken.action">待收货</a>
								</li>
								<li <c:if test="${type == 4}">class="first"</c:if>>
									<a href="getGroupOrderClosed.action">已关闭</a>
								</li>
													...
													<tr>
														<th class="col-main">
															<p class="caption-info">
																<fmt:formatDate type="both" dateStyle="long" timeStyle="medium" value="${order.createDate}" /> |订单号：${order.orderId}
															</p>
														</th>
														<th class="col-sub">
																...
																<c:forEach items="${order.products}" var="product">
																	<li>
																		<div class="goods-image">
																			<a href="selectProductInfo.action?productName=${product.product.productName}" target="_blank"> <img src="../${product.product.picUrl}" width="80" height="80">
																			</a>
																		</div>
																		<p class="name">
																		...
																		<p class="price">${product.productPrice}元× ${product.quantity}
																		</p>
																	</li>
																</c:forEach>
															</ul>
							...
							%>
								<ul class="pagination pagination-lg ">
									<li <c:if test="${page eq 1}">class="disabled "</c:if>>
										<a href='<c:if test="${page != 1}"><c:if test="${type == 1}">getAllGroupOrder</c:if><c:if test="${type == 2}">getGroupOrderWaitPaid</c:if><c:if test="${type == 3}">getOrderGroupWaitTaken</c:if><c:if test="${type == 4}">getGroupOrderClosed</c:if><c:if test="${type == 5}">getGroupOrderWaitBuilt</c:if>.action?page=${page-1}</c:if>'><i class="fa fa-angle-left "></i></a>
									</li>
									...
									<li <c:if test="${page eq pageTotal}">class="disabled "</c:if>>
										<a href='<c:if test="${page != 1}"><c:if test="${type == 1}">getAllGroupOrder</c:if><c:if test="${type == 2}">getGroupOrderWaitPaid</c:if><c:if test="${type == 3}">getOrderGroupWaitTaken</c:if><c:if test="${type == 4}">getGroupOrderClosed</c:if><c:if test="${type == 5}">getGroupOrderWaitBuilt</c:if>.action?page=${page+1}</c:if>'><i class="fa fa-angle-right "></i></a>
									</li>
			...
			<div class="container">
				...
				<div class="footer-links">
					...
					<div class="footer-links-contact">
						...
						<a href="" class="contactA"> <i class="fa fa-commenting-o"></i> 联系客服
						</a>
					</div>
				</div>
			</div>
			...
			<div class="container">
				...
				<div class="info-text">
					...
					<p>
						...
						<a rel="nofollow" href="" target="_blank">京网文[2017]1530-131号</a> <br> 违法和不良信息举报电话：185-0130-1238，本网站所列数据，除特殊说明，所有数据均出自我司实验室测试
					</p>
				</div>
