						...
						<div class="head-details">
							<ul class="head-list">
								<li <c:if test="${type == 1}">class="first"</c:if>>
									<a href="getAllOrder.action">全部有效订单</a>
								</li>
								<li <c:if test="${type == 2}">class="first"</c:if>>
									<a id="J_unpaidTab" href="getOrderWaitPaid.action">待支付</a>
								</li>
								<li <c:if test="${type == 3}">class="first"</c:if>>
									<a id="J_sendTab" href="getOrderWaitTaken.action">待收货</a>
								</li>
								<li <c:if test="${type == 4}">class="first"</c:if>>
									<a href="getOrderClosed.action">已关闭</a>
								</li>
							</ul>
							<form id="searchOrder" class="search-order" action="getOrderByProductName.action" method="post">
								<input class="search-text" type="search" id="" name="productName" autocomplete="off" placeholder="输入商品名称 搜索订单" />
													...
													<tr>
														<th class="col-main">
															<p class="caption-info">
																<fmt:formatDate type="both" dateStyle="long" timeStyle="medium" value="${order.createDate}" />|订单号：${order.orderId}</p>
														</th>
														<th class="col-sub">
															<p class="caption-price">订单金额：${order.actualTotal}<span class="num"></span>元</p>
														</th>
													</tr>
												</thead>
												<tbody>
						...
						%>
								<ul class="pagination pagination-lg ">
									<li <c:if test="${page eq 1}">class="disabled "</c:if>>
										<a href='<c:if test="${page != 1}"><c:if test="${type == 1}">getAllOrder</c:if><c:if test="${type == 2}">getOrderWaitPaid</c:if><c:if test="${type == 3}">getOrderWaitTaken</c:if><c:if test="${type == 4}">getOrderClosed</c:if>.action?page=${page-1}</c:if>'><i class="fa fa-angle-left "></i></a>
									</li>
